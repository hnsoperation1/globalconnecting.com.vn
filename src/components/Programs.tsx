    ...
    <div className={`rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-shadow duration-300 flex flex-col md:flex-row relative ${isDark ? 'bg-[#0a1f3d]' : 'bg-white'}`}>
      ...
      <div className={`flex flex-col justify-center p-8 flex-1 ${isEven ? 'md:order-2' : 'md:order-1'}`}>
        ...
        <p className={`text-sm leading-relaxed mb-4 line-clamp-3 ${isDark ? 'text-white/90' : 'text-gray-600'}`}>
          ...
          <Link href={`/chuong-trinh-da-lam/${p.slug}`} className={`font-medium transition-colors ${isDark ? 'text-white/50 hover:text-white/90' : 'text-gray-400 hover:text-gray-700'}`}>
            xem thêm
          </Link>
        </p>
